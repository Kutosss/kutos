...
public sealed class RoleBanWebhooks : BaseWebhookService
{
    ...
    public void OnRoleBanAdded(string targetName, string role, string adminName, string reason, DateTimeOffset banTime, DateTimeOffset? expirationTime)
    {
        ...
        var webhookUrl = _cfg.GetCVar(CCVars.DiscordWebhookUrl);
        if (!_cfg.GetCVar(CCVars.DiscordWebhookEnabled) || string.IsNullOrEmpty(webhookUrl))
        {
            LogDebug("Discord webhook is disabled or URL is not set. Skipping roleban notification.");
            return;
        }
        ...
        Task.Run(async () => {
            try
            {
                ...
                await SendRoleBanWebhook(targetName, role, adminName, reason, banTime, expirationTime, webhookUrl);
    ...
    private async Task SendRoleBanWebhook(string targetName, string role, string adminName, string reason, 
        DateTimeOffset banTime, DateTimeOffset? expirationTime, string webhookUrl)
    {
        ...
        await Webhook.CreateMessageWithToken(webhookUrl, payload);
    }
