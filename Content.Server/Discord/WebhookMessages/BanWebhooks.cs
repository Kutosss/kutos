...
public sealed class BanWebhooks : BaseWebhookService
{
    ...
    private void OnBanAdded(ServerBanDef ban)
    {
        ...
        {
            LogDebug("Discord webhook is disabled or URL is not set. Skipping ban notification.");
            return;
        }
        ...
        Task.Run(async () => {
            try
            {
                await SendBanWebhook(ban, webhookUrl);
    ...
    /// </summary>
    /// <param name="ban">Информация о бане</param>
    /// <param name="webhookUrl">URL вебхука Discord</param>
    private async Task SendBanWebhook(ServerBanDef ban, string webhookUrl)
    {
